...
@MainActor class GridLayout {
    ...
    func registerNumberOfCells(_ numberOfCells: Int) {
        if numberOfCells != _numberOfCells {
            print("🤹🏽‍♀️ [Layout] Registered #\(numberOfCells) Cells!")
    ...
    private func layoutGrid() {
        ...
        print("_numberOfCols = \(_numberOfCols) _numberOfRows = \(_numberOfRows) _numberOfCells = \(_numberOfCells)")
        
        _cellWidthArray = cellWidthArray()
        _cellXArray = cellXArray()
        
        if _cellWidthArray.count > 0 {
            let cellWidth = _cellWidthArray[0]
...
extension GridLayout {
    ...
    func cellXArray() -> [Int] {
        ...
        for index in 0..<_numberOfCols {
            spaceConsumed += _cellWidthArray[index]
            if (index < (_numberOfCols - 1)) {
                spaceConsumed += cellSpacingH
            }
        }
